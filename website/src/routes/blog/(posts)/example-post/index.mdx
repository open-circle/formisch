...
If you use a validation schema alongside your default values, which most teams do, those two things still have no enforced connection at the Typescript level. A field added to one but not the other will not be caught. The same silent failure that applies to RHF applies here, just between fewer moving parts.
