...
- **RHF stores values in RHF's internal object, updating the DOM directly**. Components that need to react to value changes subscribe through watch, which triggers re-renders.
...
[RHF asks you to declare the form's shape as a TypeScript type and pass it in as a generic](https://react-hook-form.com/docs/useform): `useForm<MyFormValues>()`. That generic is a TypeScript-only construct. It exists purely at compile time and is completely absent at runtime.
...
[When validation runs is controlled by a single `mode` option set on the form](https://react-hook-form.com/docs/useform): `onSubmit`, `onBlur`, `onChange`, `onTouched`, or `all`. That mode applies to every field in the form. There is no way to say "validate this field on blur but validate that one on change" without writing custom `onChange` handlers yourself.
...
Formisch centralizes everything in the Valibot schema. The component registers a field name and everything else comes from the schema, without field-level validators, form-level validator configuration, or resolver setup.
...
The tradeoff appears when React needs to know about a field's value. Conditional rendering, UI updates as you type, and cross-field rules all require React to be aware of the current value. The mechanism for that is [`watch`](https://react-hook-form.com/docs/useform/watch), which opts a field into React state and causes re-renders when that field changes.
...
- `watch("field")` is replaced by [`getInput(form, ["field"])`](https://formisch.dev/methods/api/getInput/) when you need access outside the field component.
- `useFieldArray` is replaced by `insert`, `remove`, `move`, `swap`, and `replace` methods on the form, used alongside the [`FieldArray`](https://formisch.dev/react/api/FieldArray/) component for dynamic field arrays.
