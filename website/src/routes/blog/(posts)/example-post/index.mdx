...
- `useFieldArray` is replaced by Formisch's [`useFieldArray`](https://formisch.dev/react/api/useFieldArray) component used alongside `insert`, `remove`, `move`, `swap`, and `replace` methods on the form.
...
| Dynamic arrays | `useFieldArray` | `insert`, `remove`, `move`, `swap`, `replace` + `FieldArray` or `useFieldArray`|
