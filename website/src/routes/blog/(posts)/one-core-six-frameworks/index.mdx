...
> **Note:** This post was written when Formisch supported six frameworks. Angular and React Native have been added since, so the framework lists and the adapter file names below are no longer complete. The design they describe is unchanged: supporting a new framework still means adding one sibling module that implements the same four functions.
