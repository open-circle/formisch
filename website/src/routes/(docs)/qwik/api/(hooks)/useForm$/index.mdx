...
```ts
const form = useForm$<TSchema>(configFn);
```
...
- `configFn` <Property {...properties.configFn} />

### Explanation

`useForm$` creates a reactive form store that manages form state using the provided Valibot schema. The `configFn` callback must return a <Link href="../FormConfig/">`FormConfig`</Link> object. By default, validation runs when the form is submitted and then revalidates on every input, but this can be customized with the `validate` and `revalidate` options.
