...
`useForm$` creates a reactive form store that manages form state using the provided Valibot schema. The `configFn` callback must return a <Link href="../FormConfig/">`FormConfig`</Link> object. Validation runs when a field is blurred or the form is submitted by default, but this can be customized with the `validate` and `revalidate` options.
