...
The HTML `<input type="file" />` element is an exception because it cannot be controlled. However, you have the possibility to control the UI around it. For inspiration you can use the code of our <a href={`${import.meta.env.PUBLIC_GITHUB_URL}/blob/main/playgrounds/qwik/src/components/FileInput.tsx`} target={'\_blank'} rel="noreferrer">`FileInput`</a> component from our <Link href="/playground/special/">playground</Link>.
