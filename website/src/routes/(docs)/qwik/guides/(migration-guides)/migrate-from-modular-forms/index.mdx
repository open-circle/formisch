...
Also note that fields with a non-string schema like `v.number()` or `v.boolean()` must be controlled, since Formisch has no `type` prop that decodes the element value. See the <Link href="/qwik/guides/add-form-fields/">add form fields</Link> and <Link href="/qwik/guides/controlled-fields/">controlled fields</Link> guides for details.
...
| `type` prop (`'number'`, `'boolean'`, ...)          | Inferred from schema (non-string fields are controlled)                                  |
...
```tsx
import { Field, FieldArray, insert, remove } from '@formisch/qwik';
...
Modular Forms decodes checkboxes, number inputs, file inputs and dates through the `type` prop on `Field`. Formisch connects fields through `field.props` and leaves the element markup to you, so non-string fields are controlled with the appropriate attribute, like `checked` for a boolean checkbox:
...
The <Link href="/qwik/guides/special-inputs/">special inputs</Link> guide shows the equivalent patterns for checkbox groups, radio groups, selects, file inputs and dates.
