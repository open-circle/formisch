...
- **`formState.touchedFields`**: Formisch marks a field as touched when it receives focus or its input is set, while React Hook Form marks it on blur. During ordinary keyboard interaction that means touched-based logic usually reacts on focus instead of waiting for blur. Formisch's `setInput` also marks the field as touched and edited; React Hook Form's `setValue` only updates those flags when `shouldTouch` or `shouldDirty` is enabled.
