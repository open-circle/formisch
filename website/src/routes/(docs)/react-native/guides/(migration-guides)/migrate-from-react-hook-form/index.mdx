...
Translate your `Controller` rules or resolver schema into a single Valibot schema. Most rules have a close counterpart: `required` becomes `v.nonEmpty()` for strings and arrays, `minLength` becomes `v.minLength()`, and `pattern` usually becomes a dedicated action like `v.email()` or `v.regex()`. Use a type-appropriate schema to express requiredness for other inputs.
...
- **`formState.touchedFields`**: Formisch marks a field as touched when it receives focus or its input is set, while React Hook Form marks it on blur. During ordinary interaction that means touched-based logic usually reacts on focus instead of waiting for blur. Formisch's `setInput` also marks the field as touched and edited; React Hook Form's `setValue` only updates those flags when `shouldTouch` or `shouldDirty` is enabled.
