...
Translate your Yup schema, `validate` function, or field-level `validate` props into a single Valibot schema. Most Yup rules have a close counterpart: string `.required()` becomes `v.nonEmpty()`, string `.min()` becomes `v.minLength()`, `.email()` becomes `v.email()`, and `.matches()` becomes `v.regex()`. Other input types need their corresponding Valibot schema and actions.
...
- **`touched`**: Formisch marks a field as touched when it receives focus or its input is set, while Formik marks it on blur. During ordinary interaction that means touched-based logic usually reacts on focus instead of waiting for blur.
...
- **`setFieldTouched` / `setTouched`**: There is no imperative touched setter. Fields become touched automatically when they receive focus or their input is set, and the custom blur handlers these helpers are typically used in are replaced by spreading `field.props`. This also means Formisch's `setInput` marks the field as touched and edited, whereas Formik's `setFieldValue` does not mark it as touched.
