...
The form store also provides other reactive properties (signals) like `isSubmitted`, `isValidating`, `isTouched`, `isDirty`, `isValid`, and `errors` for tracking form state. Note that `errors.value` only contains validation errors at the root level of the form — to get all errors from all fields, use the <Link href="/methods/api/getDeepErrors/">`getDeepErrors`</Link> method.
...
```tsx
import { submit } from '@formisch/preact';
