...
Dynamically generating form fields from an array introduces additional challenges when items are added, removed, swapped, or moved. Formisch provides the <Link href="/preact/api/FieldArray/">`FieldArray`</Link> component, which works with its array methods to make these forms straightforward.
...
To dynamically generate the form fields for the todos, you use the <Link href="/preact/api/FieldArray/">`FieldArray`</Link> component in combination with JavaScript's `Array.prototype.map()` method. The field array provides an `items` signal that contains unique string identifiers which are used to detect when an item is added, moved, or removed.
