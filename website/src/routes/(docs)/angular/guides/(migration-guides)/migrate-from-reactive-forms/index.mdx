...
| `Validators.required`                  | `v.nonEmpty()` for strings and arrays; otherwise a type-appropriate schema                         |
...
| `updateOn: 'change'`                   | `validate: 'input'` on `injectForm`                                                                |
...
| `form.dirty`                           | `form.isEdited()` (see below)                                                                      |
...
A few mappings have slightly different semantics. Angular's `dirty` flag stays set after a user change, even when the value is changed back, so it maps to Formisch's `isEdited()`. Use `isDirty()` when you instead need to know whether the current input differs from the initial input. Angular marks a control as touched on blur, while Formisch does so when it receives focus or its input is set. Calling `setInput` also marks the affected Formisch fields as touched and edited, unlike Angular's programmatic `patchValue`. Finally, `getInput(form)` includes every schema field and is therefore closer to `getRawValue()` than to `form.value`, which excludes disabled controls.
...
<button
  type="submit"
  [disabled]="loginForm.isSubmitting() || loginForm.isValidating()"
>
  Login
</button>
```

Note that you rarely need to disable the button on `invalid` with Formisch: submitting an invalid form runs validation, shows the errors, and focuses the first invalid field, which is friendlier than a permanently disabled button. Keep the `isSubmitting()` guard to prevent duplicate submissions while an async handler runs.
