...
| `field.api.state.meta.isDirty`              | `field.isEdited()` (see below)                                                         |
| `field.api.state.meta.isDefaultValue`       | `!field.isDirty()` (see below)                                                         |
| `injectStore(form, (s) => s.isSubmitting)`  | `form.isSubmitting()`                                                                  |
| `injectStore(form, (s) => s.canSubmit)`     | No direct equivalent (see below)                                                       |
...
- **`canSubmit`**: Formisch does not compute a combined flag. The idiomatic pattern is to keep the button enabled, let submission trigger validation, and disable it with `form.isSubmitting()` to prevent duplicates. If you need a validity-based flag, `form.isValid()` reflects the last validation run; with the default `validate: 'submit'` timing it stays `true` until the first submit, so use `validate: 'initial'` for proactive validity.
- **Dirty state**: TanStack Form's `isDirty` stays `true` once a field was changed, even if its value is reverted. That matches Formisch's `isEdited()`. Formisch's `isDirty()` compares current and initial input, matching the inverse of TanStack Form's `isDefaultValue`.
