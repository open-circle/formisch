...
The field array store exposes the following properties:

- `path`: The path to the array field within the form.
- `items`: A signal with the item IDs of the array field.
- `errors`: A signal with the error messages of the field array itself.
- `isTouched`, `isEdited`, `isDirty`, `isValid`: Signals with the aggregated state of the array and its items.
