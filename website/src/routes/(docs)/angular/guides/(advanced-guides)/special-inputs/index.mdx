...
For the HTML `<input type="file" />` element it works similarly to the HTML `<select />` element. Without the `multiple` attribute it represents a single file and with, a list of files.
