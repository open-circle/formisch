...
> Validation always runs against the whole schema, even when a single field triggers it. This keeps cross-field rules (like a `password` and `confirmPassword` that must match) correct without any extra wiring. The result is still stored per field, so each `<input />` only updates when its own errors change.
...
The <Link href="/angular/api/formischControl/">`[formischControl]`</Link> directive wires the `input`, `change`, `focus` and `blur` events of the element for you, so every mode works without extra bindings.
...
Each field exposes several state signals to drive this decision:
    ...
    <form [formischForm]="loginForm" [formischSubmit]="handleSubmit">
      <ng-container *formischField="['email'] of loginForm; let field">
        ...
        @if (field.errors(); as errors) {
          @if (field.isEdited() || loginForm.isSubmitted()) {
            <div>{{ errors[0] }}</div>
          }
        }
      </ng-container>
...
Writing this guard at every field gets repetitive. The common fix is to move it into a small reusable component, either on its own or folded into the `TextInputComponent` from the <Link href="/angular/guides/input-components/">input components</Link> guide, so the `isEdited() || isSubmitted()` logic lives in exactly one place:
...
export class FieldErrorComponent {
  ...
  readonly isEdited = input.required<boolean>();
...
@Component({
  ...
  template: `
    <form [formischForm]="signUpForm" [formischSubmit]="handleSubmit">
      <!-- fields -->
...
export class SignUpComponent {
  ...
  readonly handleSubmit = (output: v.InferOutput<typeof SignUpSchema>) => {
    console.log(output);
  };
}
```
...
The <Link href="/angular/api/formischForm/">`[formischForm]`</Link> directive adds the `novalidate` attribute to your `<form>` element, so the browser's own validation bubbles never compete with your schema's error messages. Attributes like `required` on your inputs stay useful for accessibility and styling.

## Further reading

To process your form's values once they pass validation, see the <Link href="/angular/guides/handle-submission/">handle submission</Link> guide. For working with the fields a user has changed, see the <Link href="/angular/guides/dirty-fields/">dirty fields</Link> guide.
