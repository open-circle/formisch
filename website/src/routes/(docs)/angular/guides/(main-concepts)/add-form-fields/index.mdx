...
All reactive state is exposed as signals, so you call it like a function in your template (e.g. `field.errors()`). The `path` property is a plain value, and `setInput` is a method.
