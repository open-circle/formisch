...
Also note that fields with a non-string schema like `v.number()` or `v.boolean()` must be controlled, since Formisch has no `type` prop that decodes the element value. See the <Link href="/solid/guides/add-form-fields/">add form fields</Link> and <Link href="/solid/guides/controlled-fields/">controlled fields</Link> guides for details.
...
The <Link href="/solid/guides/special-inputs/">special inputs</Link> guide shows the equivalent patterns for checkbox groups, radio groups, selects, file inputs and dates.
