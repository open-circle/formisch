...
| `$tainted` / `isTainted('email')`          | `field.isEdited` / <Link href="/methods/api/isEdited/">`isEdited`</Link>                |
...
- **`taintedMessage`**: There is no built-in navigation guard. You can build one with SvelteKit's `beforeNavigate` and the <Link href="/methods/api/isEdited/">`isEdited`</Link> method. Unlike Superforms, Formisch does not automatically clear edited state after a successful server response; call <Link href="/methods/api/reset/">`reset`</Link> when the saved values should become the new baseline.
...
Superforms spreads form state across several stores like `$submitting`, `$posted`, and `$tainted` with the `isTainted` helper. In Formisch, the form store exposes reactive properties directly: `isSubmitting`, `isSubmitted`, `isValidating`, `isTouched`, `isEdited`, `isDirty`, `isValid`, and `errors`. Each field offers per-field `isTouched`, `isEdited`, `isDirty`, and `isValid` counterparts. Superforms' tainted state is sticky after a modification, so `isEdited` is the closest match; use `isDirty` only when you want a current-versus-initial comparison that turns off after a value is reverted.

```svelte
<button type="submit" disabled={loginForm.isSubmitting || !loginForm.isEdited}>
  {loginForm.isSubmitting ? 'Saving...' : 'Save'}
</button>
```

To check the edited state of a single field outside of a <Link href="/svelte/api/Field/">`Field`</Link> component, like `isTainted('email')` in Superforms, use the <Link href="/methods/api/isEdited/">`isEdited`</Link> method. Wrap the call in `$derived` so it stays reactive:

```ts
import { isEdited } from '@formisch/svelte';

const emailIsEdited = $derived(isEdited(loginForm, { path: ['email'] }));
```
...
Superforms remembers that a field previously had errors and continues validating it on input even after the errors clear. Formisch revalidates on input while a field has errors or after the form was submitted, then returns to the initial trigger after an error clears. The explicit modes map directly: `'oninput'` to `validate: 'input'`, `'onblur'` to `validate: 'blur'`, and `'onsubmit'` to `validate: 'submit'`, which is the Formisch default. Note that `validate` and `revalidate` control when validation runs, not when errors are shown. The <Link href="/svelte/guides/validation/">validation</Link> guide shows how to reveal errors at the right time using flags like `field.isEdited` and `form.isSubmitted`.
