...
`createForm` creates a reactive form store that manages form state using the provided Valibot schema. Validation runs when the form is submitted by default, but this can be customized with the `validate` and `revalidate` options.
