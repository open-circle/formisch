 ...
 * Field element type.
 *
 * Hint: React Native has no DOM elements. This is a structural subset of the
 * imperative methods that React Native host component instances provide, so
 * registered refs of `TextInput` and other native components are accepted
 * without depending on the `react-native` package. `isFocused` is only
 * implemented by `TextInput` and is used to verify that an element actually
 * received focus.
 */
